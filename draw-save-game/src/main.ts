...
import { persistFormalCompletion } from './game/formalCompletion';
...
class RescueScene extends Phaser.Scene {
  ...
  create() {
    ...
    const instruction = this.level.objective === 'reach' ? '畫路線，讓人物到達綠色出口' : this.level.objective === 'catch' ? '先畫承接線，把人物接在藍色救援區' : '按住並畫出防護線';
  ...
  private switchLevel(delta: number) { this.scene.restart({ levelIndex: Phaser.Math.Wrap(this.levelIndex + delta, 0, levels.length) }); }
  private startDrawing(pointer: Phaser.Input.Pointer) {
    ...
    this.drawing = true; this.currentPoints = [{ x: pointer.x, y: pointer.y }]; this.preview.clear();
  }
  private continueDrawing(pointer: Phaser.Input.Pointer) {
    ...
    this.currentPoints.push(step.next); this.inkLeft = step.inkLeft; this.drawPreview(); this.refreshHud();
    if (this.inkLeft <= 0) this.finishDrawing();
  }
  private drawPreview() {
    this.preview.clear().lineStyle(LINE_WIDTH, 0x172033, 1).beginPath();
    const first = this.currentPoints[0]; this.preview.moveTo(first.x, first.y);
  ...
  private finishDrawing() {
    ...
    for (let i = 1; i < this.currentPoints.length; i++) {
      const a = this.currentPoints[i - 1], b = this.currentPoints[i];
      const length = Phaser.Math.Distance.Between(a.x, a.y, b.x, b.y), angle = Phaser.Math.Angle.Between(a.x, a.y, b.x, b.y);
  ...
  private spawnHazards() {
    for (const spawn of this.level.hazards) {
      ...
      const hazard = isLaser ? this.matter.add.rectangle(spawn.x, spawn.y, spawn.axis === 'x' ? spawn.length : spawn.radius * 2, spawn.axis === 'y' ? spawn.length : spawn.radius * 2, { isStatic: true, isSensor: true }) : this.matter.add.circle(spawn.x, spawn.y, spawn.radius, { restitution, frictionAir, isStatic: spawn.kind === 'spike' || spawn.kind === 'mover' });
      ...
      if (isLaser) {
        const width = spawn.axis === 'x' ? spawn.length : spawn.radius * 2, height = spawn.axis === 'y' ? spawn.length : spawn.radius * 2;
        ...
        this.hazards.push({ body: hazard, kind: spawn.kind, spawnX: spawn.x, spawnY: spawn.y, axis: spawn.axis, phaseMs: spawn.phaseMs ?? 0, laserLength: spawn.length, warningMs: spawn.warningMs, activeMs: spawn.activeMs, cooldownMs: spawn.cooldownMs, laserPhase: 'warning', laserVisual });
        continue;
      }
      ...
      if (spawn.kind === 'mover') { const marker = this.add.text(spawn.x, spawn.y, spawn.axis === 'x' ? '↔' : '↕', { fontFamily: 'system-ui', fontSize: `${Math.max(14, spawn.radius)}px`, color: '#ecfeff', fontStyle: 'bold' }).setOrigin(0.5); this.events.on('update', () => marker.setPosition(hazard.position.x, hazard.position.y)); }
      this.events.on('update', () => visual.setPosition(hazard.position.x, hazard.position.y));
      this.hazards.push({ body: hazard, kind: spawn.kind, speed: spawn.kind === 'chaser' ? spawn.speed : undefined, spawnX: spawn.x, spawnY: spawn.y, speedY: spawn.kind === 'falling' ? spawn.speedY : undefined, driftX: spawn.kind === 'falling' ? spawn.driftX : undefined, intervalMs: spawn.kind === 'falling' ? spawn.intervalMs : undefined, nextResetAt: spawn.kind === 'falling' ? this.time.now + spawn.intervalMs : undefined, axis: spawn.kind === 'mover' ? spawn.axis : undefined, range: spawn.kind === 'mover' ? spawn.range : undefined, moveSpeed: spawn.kind === 'mover' ? spawn.speed : undefined, phaseMs: spawn.kind === 'mover' ? (spawn.phaseMs ?? 0) : undefined });
    }
  }

  private onCollisionStart(event: Phaser.Physics.Matter.Events.CollisionStartEvent) {
    for (const pair of event.pairs) { const hitHero = pair.bodyA === this.hero || pair.bodyB === this.hero; const other = pair.bodyA === this.hero ? pair.bodyB : pair.bodyA; if (hitHero && this.hazards.some((hazard) => hazard.body === other)) this.finishRound(false); }
  }
  ...
  update(time: number) {
    ...
    for (const hazard of this.hazards) {
      if (hazard.kind === 'chaser') this.matter.body.setVelocity(hazard.body, velocityToward(hazard.body.position, this.hero.position, hazard.speed ?? 1.5));
      if (hazard.kind === 'falling' && fallingResetDue(time, hazard.nextResetAt ?? Number.POSITIVE_INFINITY)) { this.matter.body.setPosition(hazard.body, { x: hazard.spawnX ?? hazard.body.position.x, y: hazard.spawnY ?? 100 }); this.matter.body.setVelocity(hazard.body, fallingVelocity(hazard.speedY ?? 2.5, hazard.driftX ?? 0)); hazard.nextResetAt = time + (hazard.intervalMs ?? 2500); }
      if (hazard.kind === 'mover') { const offset = oscillatingOffset(elapsed + (hazard.phaseMs ?? 0), hazard.range ?? 60, hazard.moveSpeed ?? 70); this.matter.body.setPosition(hazard.body, { x: (hazard.spawnX ?? hazard.body.position.x) + (hazard.axis === 'x' ? offset : 0), y: (hazard.spawnY ?? hazard.body.position.y) + (hazard.axis === 'y' ? offset : 0) }); }
      if (hazard.kind === 'laser') { const phase = laserPhaseAt(elapsed, hazard.warningMs ?? 900, hazard.activeMs ?? 700, hazard.cooldownMs ?? 1500, hazard.phaseMs ?? 0); if (phase !== hazard.laserPhase) hazard.laserPhase = phase; const active = phase === 'active'; this.matter.body.setPosition(hazard.body, active ? { x: hazard.spawnX ?? 0, y: hazard.spawnY ?? 0 } : { x: -1000, y: -1000 }); const fill = active ? 0xef4444 : phase === 'warning' ? 0xf59e0b : 0x94a3b8; const stroke = active ? 0x991b1b : phase === 'warning' ? 0xb45309 : 0x475569; const alpha = active ? 0.9 : phase === 'warning' ? 0.4 : 0.12; hazard.laserVisual?.setFillStyle(fill, alpha).setStrokeStyle(2, stroke, Math.max(alpha, 0.35)); }
    }
    const remaining = Math.max(0, this.level.surviveMs - elapsed); this.timerText.setText(`${(remaining / 1000).toFixed(1)}s`);
    if (this.level.objective === 'survive') { if (elapsed >= this.level.surviveMs) this.finishRound(true); return; }
    const target = this.level.target!;
    if (pointInsideTarget(this.hero.position, target)) { if (!this.targetEnteredAt) this.targetEnteredAt = time; const heldMs = time - this.targetEnteredAt; const neededMs = target.holdMs ?? 700; const label = this.level.objective === 'catch' ? '救援區穩定' : '出口確認'; this.statusText.setText(`${label} ${(Math.min(heldMs, neededMs) / neededMs * 100).toFixed(0)}%`); if (heldMs >= neededMs) this.finishRound(true); } else this.targetEnteredAt = 0;
...
mountLevelSelect({ levels, onSelect: (_level, levelIndex) => { game.scene.getScene('rescue').scene.restart({ levelIndex }); } });
mountLevelEditorPanel({ levels, onPreview: (level, levelIndex) => { game.scene.getScene('rescue').scene.restart({ levelIndex, customLevel: level }); } });